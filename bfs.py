...
def bfs_completo(matriz,nodos):
    blancos = [False] * len(nodos)      #marcamos todos los nodos como blancos O(n)
...
def bfs(u, matriz, blancos):
    ...
    pertenecientes = []                 #creamos una lista para ir almacenando nodos 
                                        #recorridos a partir del inicial(arbol resultante)
    
    while (not cola.empty()):               #mientras la cola no este vacia                               O(n)
        ...
        if(actual in matriz):               #si el actual esta en la matriz
            for v in matriz.get(actual):    #por cada nodo adyacente al actual (es decir por cada arco)   O(a)
